...
const classes: Record<
  ...
  string
> = {
  footer: "mx-auto w-full px-4 sm:px-6 md:px-8 lg:px-10 max-w-[1880px] flex flex-col items-center py-4",
  container: "self-stretch bg-zinc-200 rounded-[48px] inline-flex justify-between items-center",
  ...
  menuItem: "self-stretch justify-start text-stone-900 text-xl font-normal font-Syne leading-loose",
  ...
  sectionTitle: "self-stretch justify-start text-white text-3xl font-bold font-Syne uppercase",
  sectionText: "self-stretch justify-start text-white text-opacity-80 text-xl font-normal font-Syne leading-loose",
...
export default function SectionFooter() {
  ...
  const socialIcons = ["/images/Facebook.svg", "/images/Instagram.svg", "/images/LinkedIn.svg"]
      ...
      <footer className="mx-auto w-full px-4 py-4 flex flex-col items-center">
        <div className="self-stretch bg-zinc-200 rounded-[24px] flex flex-col w-full">
          ...
          <div className="px-8 py-6 flex flex-col items-center text-center gap-6">
            <div className="text-stone-900 text-2xl font-bold font-Syne uppercase">ITC Talks</div>
            <div className="flex flex-col justify-center items-center gap-2">
              {menuItems.map((item, index) => (
                <div key={index} className="text-stone-900 text-base font-normal font-Syne">
                  {item}
                </div>
              ))}
            </div>
          ...
          <div className="w-full bg-blue-700 p-8 rounded-[24px] flex flex-col items-center gap-8">
            <InfoSection title="EVENT LOCATION" content="Auditorium, Saad Dahlab University" isMobile={true} />
            <InfoSection title="FOLLOW US" isMobile={true}>
              <div className="flex justify-center items-center gap-4">
                {socialIcons.map((src, index) => (
                  <Image key={index} src={src || "/placeholder.svg"} alt="Social Icon" width={24} height={24} />
                ))}
              </div>
            </InfoSection>
            <InfoSection title="POWERED BY" isMobile={true}>
              <div className="relative h-6 w-8">
                <Image src="/images/WhiteItc.svg" alt="Powered By Logo" width={32} height={24} />
              </div>
            </InfoSection>
          </div>
        </div>
        ...
        <div className="self-stretch bg-zinc-200 rounded-[36px] flex flex-col w-full">
          ...
          <div className="px-12 py-10 flex flex-col items-start gap-10">
            ...
            <div className="flex flex-col justify-start items-start gap-3">
              {menuItems.map((item, index) => (
                <div key={index} className="text-stone-900 text-lg font-normal font-Syne">
                  {item}
                </div>
              ))}
            </div>
          ...
          <div className="w-full bg-blue-700 p-12 rounded-[36px] flex flex-col items-start gap-12">
            <InfoSection title="EVENT LOCATION" content="Auditorium, Saad Dahlab University" isTablet={true} />
            <InfoSection title="FOLLOW US" isTablet={true}>
              <div className="flex items-center gap-4">
                {socialIcons.map((src, index) => (
                  <Image key={index} src={src || "/placeholder.svg"} alt="Social Icon" width={28} height={28} />
                ))}
              </div>
            </InfoSection>
    ...
    <footer className={classes.footer}>
      <div className={classes.container}>
        <div className={classes.leftSection}>
          ...
          <div className={classes.copyright}>© 2025 Copyright ITC Talks. All rights reserved.</div>
        </div>
        <div className={classes.rightSection}>
          <InfoSection title="EVENT LOCATION" content="Auditorium, Saad Dahlab University" />
          <InfoSection title="FOLLOW US">
            <div className={classes.iconsContainer}>
              {socialIcons.map((src, index) => (
                <Image key={index} src={src || "/placeholder.svg"} alt="Social Icon" width={30} height={30} />
              ))}
            </div>
          </InfoSection>
          <InfoSection title="POWERED BY">
            <div className={classes.poweredByContainer}>
              <Image src="/images/WhiteItc.svg" alt="Powered By Logo" width={37} height={30} />
            </div>
          </InfoSection>
        </div>
      </div>
...
function InfoSection({
  ...
  if (isMobile) {
    return (
      <div className="flex flex-col justify-center items-center gap-2 text-center w-full">
        ...
        {content ? <div className="text-white text-opacity-80 text-sm font-normal font-Syne">{content}</div> : children}
      </div>
    )
  }
  ...
  if (isTablet) {
    return (
      <div className="flex flex-col justify-start items-start gap-3 w-full">
        ...
        {content ? (
          <div className="text-white text-opacity-80 text-base font-normal font-Syne">{content}</div>
        ) : (
          children
        )}
      </div>
  ...
  return (
    <div className="self-stretch flex flex-col justify-start items-start gap-4">
      ...
      {content ? <div className={classes.sectionText}>{content}</div> : children}
    </div>
  )
}
