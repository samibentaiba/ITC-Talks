  ...
  return (
    <header className="container mx-auto flex items-center justify-between py-4 px-4 md:px-6">
      <nav className="hidden md:flex space-x-6">
        {navItems.map((text: string, i: number): ReactElement => (
          <Link
            key={i}
      ...
      <button className="md:hidden text-white" aria-label="Menu">
        <svg
          ...
          strokeWidth={2}
        >
          {menuLines.map((y: number, i: number): ReactElement => (
            <line
              key={i}
              x1={3}
              y1={y}
      ...
      <h1 className="text-xl md:text-2xl font-bold tracking-wider flex-1 text-center">
        ITC TALKS
      </h1>
